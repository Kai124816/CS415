...
void initialize_monitor() {
    int* tid = malloc(sizeof(int));
    *tid = 1;
    pthread_create(&monitor_thread, NULL, monitor_routine, tid);
}
...
void print_simulation_state()
{
    ...
    printf("\n");
    printf("[Monitor] System State at ");
    print_elapsed_time();
    printf("\n");
    buffer = queue_buffer(ticket_queue);
    printf("Ticket queue %s", buffer);
    free(buffer);
    buffer = queue_buffer(ride_queue);
    printf("Ride queue %s", buffer);
    free(buffer);
    for(int i=0;i<num_cars;i++)
    {
        buffer = car_state_buffer(car_array[i],i);
        printf("%s", buffer);
        free(buffer);
    }
    buffer = passenger_state_buffer(car_array,ticket_queue,ride_queue,in_park,num_cars);
    free(buffer);
...
void initializer()
{
    ...
    initialzie_car_util(util,num_cars);
}
